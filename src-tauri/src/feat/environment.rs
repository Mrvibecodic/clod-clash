...
///
/// clod:net-virtual — Docker, Podman, LXD/Incus, VirtualBox, VMware, Parallels
/// и гипервизор Apple поднимают и гасят свои адаптеры по команде пользователя;
/// к пути машины наружу это отношения не имеет. Без этого `docker compose down`
/// или `podman machine stop` рвали все живые соединения ровно так же, как
/// мигание Teredo. Имена, которые выбирает человек (`vEthernet (…)` у WSL и
/// Hyper-V), разбирает соседняя половина предиката — метка там сравнивается
/// целиком, иначе под правило попал бы коммутатор, названный администратором.
