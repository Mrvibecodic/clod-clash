...
/// То, что показывать пользователю: туннель заявлен ядру прямо сейчас.
///
/// clod:tray-fact — трей рисовал галочку, иконку и подсказку по
/// `enable_tun_mode`, то есть по ЖЕЛАНИЮ. При подавленном TUN главный экран
/// честно говорил «не запустился», а в трее в тот же момент стояла галочка —
/// два ответа на один вопрос.
///
/// Желание передаётся аргументом, а не читается здесь: и трей, и `get_tun_state`
/// уже держат в руках снимок `IVerge`, и второй поход в конфиг за тем же
/// значением им не нужен.
