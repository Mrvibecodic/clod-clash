...
// clod: у апстрима эти четыре функции живут в utils/server.rs и manager/state.rs,
// которых у нас нет; оживут при следующем портировании — файл намеренно держим
// байт-в-байт близким к апстриму.
#[cfg(windows)]
#[allow(dead_code)]
