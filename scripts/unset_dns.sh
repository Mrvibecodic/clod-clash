...
# clod: восстанавливаем DNS на том сервисе, на котором подменяли, а не на
# текущем — их могло стать два (Wi-Fi ↔ Ethernet). Первая строка файла
# состояния — имя сервиса, остальное — исходные адреса. Старый формат (только
# адреса) тоже понимаем: тогда берём текущий сервис, как раньше.
...
if [[ "$first_line" == "empty" || "$first_line" =~ ^[0-9a-fA-F:.]+$ ]]; then
    # Старый формат: в файле только адреса.
