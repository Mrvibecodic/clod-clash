...
| `profile-logo` | ссылка на логотип провайдера | скачивается при обновлении подписки и хранится локально: логотип не мигает при старте, работает офлайн и не тянется с чужого хоста при каждом показе экрана. Принимается только ответ `image/*` не тяжелее 2 МБ. Только `https` |
...
пустую группу ядро не примет, а `DIRECT` пустил бы трафик мимо туннеля.
...
что и в логах, значения `secret`, `token`, `password`, `uuid`, `x-hwid` заменяются на `***`,
и любое «слово» длиннее 24 символов из букв и цифр — тоже. Это осознанно грубо: лучше
замазать лишнее, чем отдать в чат токен подписки.

Уровень логов приложения по умолчанию — `debug`: отчёт полезен ровно настолько, насколько
подробен лог, который в него попал. Ротация ограничивает это 1 МБ на файл и восемью файлами,
уровень меняется в Настройки → Ядро.
