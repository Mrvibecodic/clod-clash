...
* **Выбор сервера не сбрасывается** тестом задержек и обновлением подписки; избранные всплывают
  ...
  общая галка «Автоматическое закрытие соединений» отключает разрыв на обоих этих путях. Кому
  удобнее, чтобы и главная рвала соединения прежнего узла, включает рядом галку «Закрывать
  соединения при выборе сервера на главной» (по умолчанию выключена).
...
* **Неудачное обновление подписки не отбирает рабочий профиль.** Конфиг, который отверг ядро
  ...
  применено», причина показывается словами.
