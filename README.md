...
* **Выбор сервера не сбрасывается** тестом задержек и обновлением подписки; избранные всплывают
  наверх и подменяют пропавший узел. Смена сервера на главной и из трея не рвёт живые
  соединения: начатая загрузка дотягивается через прежний сервер, новые идут через выбранный
  (на странице «Прокси» разрыв по-прежнему есть — для тех, кому нужно переключиться сразу).
