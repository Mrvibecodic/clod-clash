...
| `profile-logo` | ссылка на логотип провайдера | скачивается при обновлении подписки и хранится локально: логотип не мигает при старте, работает офлайн и не тянется с чужого хоста при каждом показе экрана. Для подписки, добавленной до появления кэша, файл дотягивается один раз при первом показе. Принимаются `png`, `jpeg`, `webp`, `avif`, `gif`, `svg`, `bmp`, `ico` не тяжелее 2 МБ; ответ другого типа не сохраняется, и логотип грузится по исходной ссылке, как раньше. Только `https` |
...
пустую группу ядро не примет, а `DIRECT` пустил бы трафик мимо туннеля. Две группы
это не затрагивает: ту, что наполняется на ходу (`use:`, `include-all`,
`include-all-proxies`, `include-all-providers` — узлы в ней появятся уже в ядре), и ту,
у которой ключа `proxies` нет вовсе.
...
**Секреты в нём уже замаскированы**: адрес подписки прогоняется через тот же маскировщик,
что и в логах; значение после `secret`, `token`, `password`, `passwd`, `uuid`,
`authorization`, `api-key`, `x-hwid`, `hwid`, `sub-url` заменяется на `***` (список не
исчерпывающий и будет пополняться), как и то, что стоит после `Bearer`, `Basic` и `Token`.
Отдельно вырезается любое «слово» от 20 символов из алфавита `A–Z a–z 0–9 - _ = . + /`,
если в нём есть цифра или разный регистр, — под это подходят токены и base64, но не
обычный текст и не пути. Это осознанно грубо: лучше замазать лишнее, чем отдать в чат
токен подписки.
