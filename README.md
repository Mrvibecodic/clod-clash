...
* **Подписка грузится, даже если домен провайдера заблокирован.** Сначала как задано в подписке
  (по умолчанию напрямую), потом через уже поднятый туннель, потом через системный прокси. Понимает запасные адреса и переезд
...
* **Два ядра.** Mihomo от MetaCubeX без изменений (по умолчанию) и
  ...
  библиотека TLS-отпечатков utls v1.9.0-mod-meta (Firefox 148, Safari 26.3), узел в группах
  считается мёртвым только после второй неудачной проверки, а нестабильный узел проигрывает
  стабильному. Переключаются в Настройки → Продвинутые настройки → Настройки Clash → «Ядро
