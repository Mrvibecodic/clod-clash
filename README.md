...
| `x-hwid-not-supported` | панель требует идентификатор, а клиент его не прислал | диалог «Провайдер требует идентификацию устройства. Включить?». Важнее `x-hwid-limit`: Remnawave ставит тот в обеих ветках блокировки, и без этого приоритета пользователь получал бы сообщение о лимите, которого не достигал |
