...
Основная техническая часть Clash Verge Rev (правила, соединения, логи, редакторы конфигов)
...
* **Все заголовки ответа панели Remnawave и Happ** плюс синонимы для совместимости с другими
  клиентами и собственные `clod-*`; в запросе клиент шлёт шесть своих. Через заголовки ответа
  панель передаёт название тарифа, логотип, объявление, промо-баннер, личный кабинет, поддержку,
  бот и инструкцию, оформление, режим интерфейса и границы цвета пинга. Полный список — в [docs/HEADERS.md](./docs/HEADERS.md).
...
* **Подписка загружается через туннель**, когда прямой путь не работает: сначала напрямую, потом
  через собственное ядро, потом через системный прокси — и при добавлении через окно, и при
  обновлении (импорт по deep link идёт только напрямую).
...
* **Состояние подписки видно с одного взгляда**: активная залита акцентом, истекающая помечена
  жёлтым, истёкшая приглушена и помечена красным, исчерпанный трафик — отдельное состояние.
* **Пустой ответ панели объясняется словами.** Remnawave, когда выдавать нечего, отвечает не
  ...
  и показывает причину: «Подписка истекла», «Трафик закончился», «Превышен лимит устройств» или
  «Провайдер не выдал серверы»,
...
* **Режим TUN настраивается сам**: на Windows службу ставит установщик приложения (отдельного
  запроса прав нет ни при установке, ни при обновлении), на Linux — пакет `.deb`/`.rpm`, выбор
  пользователя не стирается, а провал
  старта виден на экране, а не только в логе.
* **Простой режим интерфейса** — одна кнопка Connect на главной; вся техническая часть Clash
  Verge Rev остаётся в расширенном (переключатель «Расширенный режим» в Настройки → Основные).
  Экран настроек одинаков в обоих режимах и тоже короткий: «Настройки системы» (TUN, системный
  прокси, подключение при запуске, автозапуск, запуск в трее), ссылки провайдера и свёрнутый блок
  «Основные» — язык, тема, расширенный режим, подгон окна под содержимое, идентификация
  устройства, отчёт для поддержки,
  версия и предварительные сборки. Остальное не отобрано, а убрано под «Продвинутые настройки» —
  раскрывающийся блок с четырьмя группами («Оформление и поведение», «Настройки Clash»,
  «Инструменты», «Обслуживание»), где лежат ядро, сеть, оформление, диагностика и входы на
  технические экраны.
* **Предварительные сборки — по выбору.** В Настройки → Основные есть галка «Предварительные
  сборки»: с ней приходят альфа- и бета-версии, без неё — только стабильные выпуски. На новых
  установках она снята; у тех, кто ставил клиент раньше, сохраняется прежнее значение.
* **Быстрые действия на главной расширенного режима**: системный прокси, TUN, подключение при
  запуске, автозапуск и запуск в трее без захода в настройки. Если способ подключения задал
  провайдер, вместо двух первых тумблеров — строка об этом.
* **Выбор сервера не сбрасывается** тестом задержек и обновлением подписки; избранные всплывают
  наверх и подменяют пропавший узел. Смена сервера на главной не рвёт живые соединения: начатая
  загрузка дотягивается через прежний сервер, новые идут через выбранный. На странице «Прокси»
  и в меню трея соединения прежнего узла закрываются — для тех, кому нужно переключиться сразу;
  общая галка «Автоматическое закрытие соединений» (Настройки → Продвинутые настройки →
  Оформление и поведение → «Прочее») отключает разрыв на обоих этих путях. Кому удобнее, чтобы и
  главная рвала соединения прежнего узла, включает там же галку «Закрывать соединения при выборе
  сервера на главной» (по умолчанию выключена, работает только при включённой общей).
* **Цвет пинга** — зелёный, жёлтый и красный с границами 200 и 400 мс; провайдер задаёт свои
  границы заголовком `clod-ping`, и они действуют на главной и на странице «Прокси».
* **Клиент говорит, что не так, а не молчит с зелёным значком.** Ядро запустилось, но не
  отвечает; порт прокси занят другим приложением; ядро упало и было перезапущено; обновление
  подписки не прошло или конфиг отвергнут; конфиг нельзя передать фоновой службе — на каждый
  случай уведомление с причиной и, где возможно, с действием.
* **Клиент не решает за пользователя.** Настройки не перезаписываются его значениями, чужой
  системный прокси не трогается, при выключенном тумблере прокси в систему ничего не пишется,
  а пустая группа узлов даёт `REJECT`, а не пускает трафик мимо туннеля.
* **Соединения не рвутся без причины.** Слушатели ядра пересоздаются только когда меняются порты,
  туннель или доступ из локальной сети; при смене сети соединения закрываются только если прежний
  путь и правда пропал; сон замечается на всех платформах, включая Windows, и после пробуждения
  зависшие соединения закрываются один раз, а в журнал пишется, сколько машина спала.
* **Неудачное обновление подписки не отбирает рабочий профиль.** Конфиг, который отверг ядро
  или который нельзя собрать для фоновой службы, откатывается, карточка помечается «не
  применено», причина показывается словами. Исключение — отказ панели по устройству: тогда
  прежние серверы убираются намеренно (имена и правила остаются, адреса и ключи стираются),
  иначе лимит устройств на этом компьютере не действовал бы.
...
* **На Windows службу ставит установщик приложения.** Он и так запускается с правами
  ...
  WebView остаются непривилегированными. На Linux службу так же регистрирует пакет `.deb`/`.rpm`.
...
* **Факт вместо обещания.** Вывод ядра разбирается: если mihomo не смог поднять устройство
  ...
  состояние туннеля, а не сохранённую настройку. Неудачный подъём получает три попытки, потом
  туннель придерживается с названной причиной; выключенный тумблер снимает устройство сразу;
  после сна туннель, придержанный из-за нехватки прав, не дёргается заново — права сном не
  лечатся.
* **Ядро под присмотром в обоих режимах.** Упавшее ядро перезапускается — до трёх попыток
  ...
  системной службой: раз в полминуты приложение спрашивает у службы, жив ли процесс ядра и
  сколько раз она его поднимала, и заодно проверяет, что ядро отвечает. Если служба
  перезапустила ядро сама, приложение возвращает выбор сервера, меню трея и системный прокси
  и сообщает о перезапуске; если ядро замолчало — перезапускает. Старт при этом
  подтверждается ответом ядра, а не фактом запуска процесса: ядро, которое поднялось и не
  отвечает, гасится с сообщением, а не остаётся зелёным.
* **Конфиг, который службе нельзя передать, не ломает старт молча.** Если в конфиге путь
  провайдера ведёт за пределы папки или два провайдера пишут в один файл, служба такой набор
  не возьмёт: ядро поднимается без неё, TUN недоступен, и на экране сказано, что исправить.
  После правки конфига и перезапуска ядра передача службе делается заново.
...
* **Три ключа туннеля всё же отданы наружу.** `stack`, `strict-route` и `dns-hijack` собираются
  ...
  «Режим TUN» положение **Auto** и означает «слушаться подписки». Что при этом легко сломать —
  в [docs/REMNAWAVE.md](./docs/REMNAWAVE.md#4-граница-ответственности-что-ваше-что-клиента);
  главное: на Windows стеки `system` и `mixed` пропускают трафик, только если входящие
  подключения ядра разрешены брандмауэром. Установщик добавляет такое правило для встроенного
  ядра, а если его нет (например, у скачанного управляемого ядра), главная предупреждает об этом
  и предлагает «Починить». Поэтому на Windows клиент берёт из шаблона только `gvisor`, а
  `system` и `mixed` там включаются лишь явным выбором пользователя в диалоге «Режим TUN».
...
* **Системный DNS возвращается даже после падения.** На macOS при TUN в режиме fake-ip системный
  DNS подменяется (галка «Подменять системный DNS» в «Настройках системы», по умолчанию включена);
...
**Подписка тянется через туннель, если домен недоступен.** Каждая загрузка подписки — и добавление
через окно, и обновление — идёт лесенкой: сначала напрямую, потом через собственное ядро, потом
через системный прокси (импорт по deep link идёт только напрямую). Когда домен провайдера
заблокирован, живой канал до него — уже поднятый туннель на прошлых узлах. Отказ панели по
устройству (лимит или требование идентификатора) окончателен: он не повторяется ни через прокси,
ни через запасные адреса — адрес доступен, отвечает сервис.

**Добавление — одно поле.** По кнопке «Добавить подписку» открывается окно, где видна только
ссылка от сервиса: название, срок, трафик и серверы приходят вместе с ней. Имя, группа, описание,
интервал обновления, User-Agent, таймаут и тумблеры живут в свёрнутом блоке «Дополнительно» — при
правке существующей подписки он раскрыт сразу, потому что заходят туда именно за ними. Своё
название хранится отдельно от названия панели: обновление его не затирает, а везде оно
показывается как «Своё (от панели)». Интервал, который панель прислала в
`profile-update-interval`, берётся из каждого ответа и в свойствах недоступен для правки;
интервал, заданный вручную, панель не трогает. Срок из `subscription-userinfo` — ещё и момент
внеочередного обновления: через полторы минуты после него подписка загружается заново (панель
переводит пользователя в «истёк» кроном, не в секунду срока), чтобы заглушки вместо узлов или
продление появились сразу, а не с очередным периодом; это одна загрузка, а не опрос, поэтому она
идёт и у подписок без интервала обновления (снятая галка «Разрешить автообновление» её
запрещает), а пропущенная за время выключенного приложения — при следующем запуске. Смена адреса подписки в карточке ничего не
сбрасывает — запасные адреса и выбранный режим остаются. Второй шаг
...
истекающая (осталось ≤ 3 дней) — жёлтой меткой «Истекает», истёкшая приглушена, обесцвечена и
помечена красным «Истекла», а исчерпанный трафик при живом сроке — отдельное состояние «Трафик
кончился» с красной полосой расхода. Активную
...
`^ClodClash` (или `^clodclash` с `caseSensitive: false`) и форматом **MIHOMO**, иначе панель
отдаст ответ по умолчанию, а приложение покажет ошибку «Панель прислала список ссылок вместо
конфигурации — она не узнала клиент…».
...
обновлениями приложения — устройство не будет регистрироваться заново. Если панель
отказала по устройству, после добавления подписки (через окно, по ссылке или deep link) и
после каждого её обновления приложение показывает окно с именем этой подписки: при лимите — с
кнопкой «Поддержка» (если панель прислала `support-url`), при выключенной идентификации — с
предложением её включить. Несколько таких окон показываются по очереди.

**Граница ответственности.** `external-controller`, `secret`, порты кроме `mixed-port` и
большая часть секции `tun` из шаблона перезаписываются настройками приложения;
`profile.store-selected` принудительно `true` — выбор сервера переживает обновления
подписки. `mode`, `mixed-port`, `log-level` и `unified-delay` идут лесенкой «выбор
пользователя → шаблон → умолчание клиента» (`rule`, `7897`, `info`, `true`); режим
пользователь выбирает для каждой подписки отдельно, а при `clod-lock-mode` действует режим
шаблона. `ipv6` задаёт шаблон. Всё остальное — включая `dns`, `hosts`, `sniffer`, правила и
группы — отдано шаблону целиком, а `tun.stack`, `tun.strict-route` и `tun.dns-hijack`
вы задаёте, пока пользователь не выбрал их сам. Полный разбор, что чьё и на чём легче всего споткнуться
...
* **Windows x64** — установщик `.exe`: ставит приложение, фоновую службу для TUN и правило
  брандмауэра для ядра. Есть и portable-архив; он сам не обновляется.
* **macOS** (Apple Silicon и Intel) — `.dmg`. Пакет подписан без сертификата Apple (ad-hoc),
  поэтому первый запуск разрешается в «Системные настройки → Конфиденциальность и
  безопасность». Без этого окна ставит одна команда в «Терминале»:
  `curl -fsSL https://mrvibecodic.github.io/clod-clash/install-macos.sh | bash` — скрипт
  выбирает сборку под процессор, скачивает последнюю обычную версию с GitHub, кладёт её в
  «Программы» и запускает.
* **Linux x86_64** — пакеты `.deb` и `.rpm`; пакет сразу регистрирует фоновую службу.

Дальше приложение обновляется само; канал выбирает галка «Предварительные сборки».

---

## Отчёт для поддержки

Настройки → Основные → **«Отчёт для поддержки»**; то же делает кнопка «Скопировать логи для
поддержки» под любым уведомлением об ошибке. В буфер обмена кладётся готовый текст:
...
отдавать её в чат поддержки нельзя, поэтому такие строки в отчёт не попадают;
сколько их выброшено, написано в самом отчёте. Исключение — ошибки соединений
(`[TCP] dial … error: …`): от них остаются узел, порт и текст ошибки, а адрес назначения,
адрес и процесс источника и значение правила вырезаются. Всё остальное — старт ядра,
разбор конфига, ошибки провайдеров — остаётся.
...
уровень меняется в Настройки → Продвинутые настройки → Оформление и поведение → «Прочее» →
«Уровень журнала приложения».

Если поддержке нужны журналы целиком, пункт «Выгрузить логи в архив» (Продвинутые настройки →
«Обслуживание») сохраняет их в zip — с той же маскировкой, что и отчёт.
