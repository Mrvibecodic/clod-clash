...
* **Подписка грузится, даже если домен провайдера заблокирован.** Сначала напрямую, потом через
  уже поднятый туннель, потом через системный прокси. Понимает запасные адреса и переезд
...
* **Два ядра.** Mihomo от MetaCubeX без изменений (по умолчанию) и Clod Core — наш форк с
  патчами. Переключаются в Настройки → Продвинутые настройки → Настройки Clash → «Ядро Clash».
...
Вместо `true` / `false` подходят `1` / `0`, `yes` / `no`, `on` / `off`. Текст с кириллицей
отдавайте как `base64:<текст в base64>`. Непонятое значение клиент считает отсутствием заголовка.
