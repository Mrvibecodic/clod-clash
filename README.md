...
| Расход трафика между обновлениями подписки | показывает только цифру панели | досчитывает сам и помечает сумму как примерную; частота опроса ядра идёт от интервала обновления подписки, а при обновлении раз в час досчёт не нужен и выключается |
| Добавление подписки | форма на десять полей, одинаковая для добавления и правки | одно поле — ссылка, остальное свёрнуто; после добавления видно, что нашлось по ней |
| Группы подписок | нет | ярлык группы на карточке и фильтр над сеткой |
| Состояние подписки на карточке | только даты мелким шрифтом | активная залита акцентом, истекающая жёлтая, истёкшая приглушена и подчёркнута красным |
...
* **Факт вместо обещания.** Вывод ядра разбирается: если mihomo не смог поднять устройство
  ...
  подряд.
* **Служба чинится только по кнопке.** Проверка её состояния больше не умеет ничего
  устанавливать: после обновления приложения, когда служба на диске старая, приложение говорит об
  этом один раз и предлагает починить в настройках. Раньше проверку дёргал каждый старт ядра — а
  на Windows старт повторяется до пяти раз, — и запрос прав всплывал по десятку раз подряд.
