...
* **Подписка грузится, даже если домен провайдера заблокирован.** Сначала как задано в свойствах
  подписки в клиенте (по умолчанию напрямую), потом через уже поднятый туннель, потом через
  системный прокси. Понимает запасные адреса и переезд подписки на новый адрес.
...
* **Срок по часам панели.** Неверное время на компьютере срок не сдвигает. Когда срок истёк,
  подписка сама обновляется, чтобы сразу показать продление или состояние «истекла» (при неудаче
  повторяет с паузами).
...
* **Два ядра.** Mihomo от MetaCubeX без изменений (по умолчанию) и
  [Clod Core](https://github.com/Mrvibecodic/clod-core) — наш форк стабильного Mihomo: новая
  ...
  ответил и тогда, а нестабильный узел проигрывает стабильному. Переключаются в Настройки →
  Продвинутые настройки → Настройки Clash → «Ядро Clash»; каждое ядро обновляется из своих
  релизов.
* **Обновляется само.** Переключатель «Предварительные сборки» в Настройки → Основные включает
  альфа- и бета-версии; без него приходят только стабильные.
...
Коротко: `pnpm install`, `pnpm prebuild`, `pnpm dev`. Подробно — в
